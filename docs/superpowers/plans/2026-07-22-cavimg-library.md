...
4. **Final whole-branch review — CRITICAL fix (commit `05e2d63`):** the element set `role="img"` in its constructor. A custom-element constructor MUST NOT add attributes — real browsers throw `NotSupportedError` on `document.createElement('cav-img')` once the element is defined (the Angular/imperative path). happy-dom does not enforce this, so all unit tests passed while the code would crash in a browser. Moved `role` to `connectedCallback` (added a regression test asserting `role` is absent at construction, present after connect). Same commit: added `prepublishOnly: tsup` (dist is gitignored, so publish must rebuild), a `./package.json` export, and made `attributeChangedCallback` remove `aria-label` (not set it to `''`) when `alt` is absent. Deferred Minors: `set src('')` full state reset, and test-file typechecking (see the SDD ledger).
